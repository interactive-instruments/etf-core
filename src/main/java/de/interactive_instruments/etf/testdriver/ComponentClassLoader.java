...
final class ComponentClassLoader extends ClassLoader implements Closeable {
	...
	private void logDebugLoadingClass(final Object caller, final String className) {
		if (logger.isDebugEnabled()) {
			logger.trace("{} : Trying to load class {}", caller.getClass().getSimpleName(), className);
		}
	}

	private void logDebugLoadingClassFailed(final Object caller, final String className, final Class clasz) {
		if (logger.isDebugEnabled()) {
			logger.trace("{} : Loading of class {} failed, trying class loader {}",
					caller.getClass().getSimpleName(), className, clasz.getSimpleName());
		}
	}
	...
	private class FindClassClassLoader extends ClassLoader {
		...
		public Class<?> findClass(String name) throws ClassNotFoundException {
			logDebugLoadingClassFailed(this, name, super.getClass());
	...
	private class ChildURLClassLoader extends URLClassLoader {
		...
		public Class<?> findClass(final String name) throws ClassNotFoundException {
			try {
				...
				logDebugLoadingClass(this, name);
				return super.findClass(name);
