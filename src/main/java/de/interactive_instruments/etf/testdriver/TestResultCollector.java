 ...
 *
 * The TestResultCollector is exposed by a test driver, injected into a test engine and consumed during a test run by a test driver adapter.
 *
 * A class that implements the TestResultCollector should realize a state machine and could throw {@link IllegalStateException}s if operations are called in the wrong order.
 *
...
public interface TestResultCollector extends BasicTestResultCollector {
    ...
    /**
     * If a Test Case depends on other TestCases this method must be called after the recording of a Test Case has been started. If any of those TestCases have the status {@link TestResultStatus#FAILED} or {@link TestResultStatus#SKIPPED}, the recording of the Test Case will be stopped by invoking {@link TestResultCollector#end(String, int)} with the status {@link TestResultStatus#SKIPPED}.
     *
     * @param testCaseIds
     *            Test Case EIDs
     *
     ...
     *
     * Note: if the status cannot be determined or the collector is on the lowest test level (Test Assertion), the status will be set to 'failed'!
     *
     * @param testModelItemId
     *            Test Model Item EID
     *
    ...
    /**
     * Returns the {@link TestResultStatus} of a non-parametrized Test Model Item. "Undefined" is returned if the Test Model Item has not yet been executed.
     *
     * @param testModelItemId
     *            Test Model Item EID
     *
    ...
    /**
     * The error limit is set as a default test run parameter. Returns true if the error limit exceeded, false otherwise. Should be used by the client to avoid constructing expensive error messages.
     *
     ...
     *
     * The underlying collector may decide if the String is written to a path and referenced from the test result document or if it is embedded into the test result document based on the size of the message.
     *
     * @param content
     *            input String
     * @param label
     ...
     *
     * Use the {@link TestResultCollector#markAttachment(String, String, String, String)} method to mark single files which should be kept and attached to the test result document.
     *
