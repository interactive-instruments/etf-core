...
/**
 *
 * A client can call the pool and create a new task which implements the TaskProgressInterface. The pool will return a Future to the Client, that might be used for calling the result ({@link Future#get()} will block and waits for the task to complete).
 *
...
public class TaskPoolRegistry<R, T extends Task<R>> {
     ...
     *
     * If the queue is full, and the number of threads is greater than or equal to maxPoolSize, the task is rejected with an {@link RejectedExecutionException}
     *
     * @since 1.1.0
     *
     * @param corePoolSize
     ...
     *
     * If the internal queue is full, and the number of threads is greater than or equal to maxPoolSize, the task is rejected with an {@link RejectedExecutionException}
     *
     * Note: since version 1.1.0, the internal queue size is three times the maxPool size. Before the queue size was equal to the maxPool size
     *
     * @since 1.0.0
     *
     * @param corePoolSize
    ...
    /**
     * Starts the task by submitting the task to the pool. The progress of the task is then accessible by calling getTaskProgress()
     *
     * @param task
     *            executable task
