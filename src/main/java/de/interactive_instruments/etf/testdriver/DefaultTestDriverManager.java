...
public class DefaultTestDriverManager implements TestDriverManager {
	...
	final private ConfigProperties configProperties = new ConfigProperties(ETF_TESTDRIVERS_DIR, ETF_ATTACHMENT_DIR);
	...
	@Override public void initSyncWithDataStorage(final DataStorage dataStorage) {

		// TODO continuous sync with data storage on ets updates

		if(!initialized) {
			throw new IllegalStateException("Driver Manager not initialized");
		}
		etsWriteDao = ((WriteDao)dataStorage.getDao(ExecutableTestSuiteDto.class));
		try {
			for (final TestDriver testDriver : loader.getTestDrivers()) {
				final Collection<ExecutableTestSuiteDto> executableTestSuites = testDriver.getExecutableTestSuites();
				etsWriteDao.addAll(executableTestSuites);
				final Collection<TestObjectTypeDto> testObjectTypes = testDriver.getTestObjectTypes();
				testObjectTypeDtoWriteDao.addAll(testObjectTypes);
			}
	...
	@Override public TestRun createTestRun(final TestRunDto testRunDto) throws InitializationException, ObjectWithIdNotFoundException, ComponentNotLoadedException, ConfigurationException {
		testRunDto.ensureValid();
		...
		trLogger.info("Preparing Test Tasks:");
		...
		for (final TestTaskDto testTaskDto : testRunDto.getTestTasks()) {
			...
			final TestDriver tD = loader.getTestDriverById(testTaskDto.getExecutableTestSuite().getId().toString());
